...
the carbon intensity here is STRONGLY and RESIDUALLY correlated (CA-ON<->NYISO 0.74,
PJM<->MISO 0.71; survives de-seasonalization). So the DRO assumption is valid here
-- the question this experiment answers is whether that correlation translates into
robust SCHEDULING value (joint covariance beating the block-diagonal/shuffled one).

Differences vs Task A (everything else identical/locked):
  * Region set = CA-ON, US-NY-NYIS, US-MIDW-MISO, US-MIDA-PJM; clock America/Toronto.
  * R2 regime uses the NEW variable-capacity constraint (3c, CFE-driven ceiling)
    instead of the thermal/PUE constraint (3b) -- we have no temperature data for
    these zones, and 3c is the Task C model addition (Decision 8). CFE is the
    training-mean carbon-free-energy field (EM `cfe_pct`), treated as data.
...
Pre-registration discipline (same as Task A):
  ...
  3. The real run reads 2025 exactly once, at the end.
...
calibrated -- see docs/decisions.md Decision 8). Treat R2 as exploratory until
calibrated.
"""
...
# ======================================================================
REGION_ORDER_TASKC = ("CA-ON", "US-NY-NYIS", "US-MIDW-MISO", "US-MIDA-PJM")
TZ_TASKC = "America/Toronto"
...
# CFE-driven ceiling (R2), fixed from the TRAINING panel -- a feasible-set
# parameter, never re-estimated per fold or peeked from the test set.
_VARCAP_CEILING: np.ndarray | None = None


# ----------------------------------------------------------------------
# Metrics / CV (identical to Task A)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Residual baselines (identical to Task A)
# ----------------------------------------------------------------------
...
def schedule_for(rho_bar, L, workloads, epsilon, alpha, regime_key) -> np.ndarray:
    ...
    kw = dict(p_max=P_MAX, alpha=alpha, ramp=np.full(R, RAMP_PER_REGION),
              region_order=REGION_ORDER_TASKC)
    if spec["deadline"]:
...
# ----------------------------------------------------------------------
# Result records (identical to Task A)
# ----------------------------------------------------------------------
...
def main() -> int:
    ...
    print("Loading Task C carbon panel + CFE field ...")
    zones = list(REGION_ORDER_TASKC)
    carbon_wide = to_wide(load_all_zones(zones))
    panel, dates = build_daily_panel(carbon_wide, region_order=zones, tz=TZ_TASKC)
    cfe_panel, cfe_dates = build_cfe_panel(zones, tz=TZ_TASKC)
    ...
    print("\n" + "=" * 96)
    print("TASK C SENSITIVITY -- spatial gap (shuf - joint) CVaR_0.95, by regime x alpha")
    ...
    csv_path = args.out_dir / f"taskC_regimes_{stamp}{suffix}.csv"
    pkl_path = args.out_dir / f"taskC_regimes_{stamp}{suffix}.pkl"
    ...
    with pkl_path.open("wb") as f:
        pickle.dump({
            ...
            "summary_rows": rows,
            "config": {
                "region_order": REGION_ORDER_TASKC, "tz": TZ_TASKC,
