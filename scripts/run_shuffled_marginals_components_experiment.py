...
# the v10 spatial effect was detectable (0.80, the Goldilocks band); the new
# sweep is over ALPHA (the inflexible fraction). This isolates "how the flexible
# fraction modulates the spatial-correlation value" at a fixed operating point.
UTILIZATION_FIXED = 0.80               # held constant (was the UTILIZATION_LEVELS sweep)
ALPHA_LEVELS = (0.30, 0.50, 0.75)      # NEW sweep: inflexible fraction (C2)
...
# --- v11 operational components (all three ON) -------------------------------
# Cap: ~90% of fleet ceiling (4 zones x 50 MW = 200 -> 180). Binds at high util,
# stays feasible. Ramp: 30% of per-cell ceiling. alpha is swept (see ALPHA_LEVELS).
P_MAX = 180.0              # C1 aggregate per-hour cap, MW
RAMP_PER_REGION = 15.0     # C3 ramp limit Delta, MW/h (same for all regions)
# intraday_shape left as None => uniform; swap in an empirical diurnal shape later.
...
def schedule_for(
    ...
    v11: all three operational components are ON. p_max and ramp come from the
    config block; alpha (the swept inflexible fraction) is passed per call.
    """
    R = rho_bar.shape[0]
...
def main():
    ...
    # Utilization is FIXED; the sweep is over ALPHA. The CVResult.utilization
    # field is repurposed to carry the alpha value so downstream
    # table/bootstrap code is unchanged.
    workloads = np.full(R, UTILIZATION_FIXED * CEILING_PER_CELL_MW * T)  # MWh/region
    ceiling = np.full((R, T), CEILING_PER_CELL_MW)
    cv_results: list[CVResult] = []
    for a in ALPHA_LEVELS:
        alpha_vec = np.full(R, a)
        print(f"\nCV for alpha {a:.2f} (util fixed {int(UTILIZATION_FIXED*100)}%) ...")
    ...
    for a in ALPHA_LEVELS:
        alpha_vec = np.full(R, a)
        cv_joint = next(r for r in cv_results if r.utilization == a and r.sigma_label == "joint")
        cv_shuf = next(r for r in cv_results if r.utilization == a and r.sigma_label == "shuf")
    ...
    # --- Bootstrap CI on joint-vs-shuf gap, per alpha ---------------------
    summary_rows = []
    for a in ALPHA_LEVELS:
        j = next(t for t in test_results if t.utilization == a and t.sigma_label == "joint")
        s = next(t for t in test_results if t.utilization == a and t.sigma_label == "shuf")
        ...
        summary_rows.append({
            "alpha": a,
            "utilization_pct": int(UTILIZATION_FIXED * 100),
        ...
        pickle.dump(
            {
                ...
                "config": {
                    ...
                    "utilization_fixed": UTILIZATION_FIXED,
                    "p_max": P_MAX,
