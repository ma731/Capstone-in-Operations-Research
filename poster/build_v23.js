...
const FIG = "figs/";
const OUT = "";
const C={navy:"00338D",cer:"0098E0",cerT:"E6F4FC",white:"FFFFFF",ink:"1A1A1A",muted:"586173",
...
// ================= BANNER =================
const tbY=0.7,tbH=4.3; rect(M,tbY,UW,tbH,C.navy); rect(M,tbY+tbH,UW,0.13,C.cer);
imgc(FIG+"ie_logo_white.png",M+0.55,tbY+0.55,4.4,3.2);
T("DATA & TOOLS",W-M-4.75,tbY+0.42,4.7,0.4,{fs:16,bold:true,color:C.cer,cs:2,align:"left"});
...
const tx=M+5.1,tw=(W-M-5.0)-0.4-tx;
T("The Price of Sophistication: When Do Spatial and\nRobust Models Pay in Carbon-Aware Scheduling?",tx,tbY+0.4,tw,2.3,{fs:47,bold:true,color:C.white,align:"center",valign:"middle",lsm:1.0});
T("A day-ahead migration scheduler and a complexity-value decision rule",tx,tbY+2.7,tw,0.6,{fs:24,bold:true,italic:true,color:"E3EEFA",align:"center"});
T([{text:"Marco Ortiz Togashi",options:{bold:true}},{text:"      Supervisor: Prof. Bissan Ghaddar      IE University, Master in Business Analytics & Data Science"}],tx,tbY+3.45,tw,0.6,{fs:20,color:C.white,align:"center"});
...
function tile(i,num,numC,fill,label,kicker,big){const x=M+i*(tw4+kg);
  rect(x,kY,tw4,kH,fill,{shadow:true}); if(big) rect(x,kY,tw4,0.2,C.gold);
  kpiGlyph(i,x+tw4/2,kY+0.62,big);
  T(num,x+0.1,kY+0.82,tw4-0.2,1.65,{fs:big?72:62,bold:true,color:numC,align:"center",valign:"middle"});
  T(label,x+0.25,kY+2.5,tw4-0.5,1.2,{fs:20,bold:big,color:big?C.goldD:C.muted,align:"center",lsm:1.05});
  T(kicker,x+0.2,kY+kH-0.6,tw4-0.4,0.5,{fs:18,bold:true,color:big?C.goldD:C.cer,align:"center",cs:2});}
...
const colBot=32.6;
function sideHead(y,kicker,head){T(kicker,sbX,y,sbW,0.46,{fs:19,bold:true,color:C.navy,cs:2});hr(sbX,y+0.5,sbW,C.navy);T(head,sbX,y+0.64,sbW,0.8,{fs:22,bold:true,color:C.cer,lsm:1.02});return y+1.42;}
...
function colHead(x,kicker,head){T(kicker,x,top,mcW,0.5,{fs:22,bold:true,color:C.cer,cs:2});T(head,x,top+0.56,mcW,1.75,{fs:33,bold:true,color:C.navy,lsm:1.02});hr(x,top+2.5,mcW,C.cer);}
...
rect(M,sbY2,UW,sbBH,C.white,{line:{color:C.line,width:1},shadow:true}); rect(M,sbY2,UW,0.16,C.cer);
T("THE RESULT, SEEN DIRECTLY",M+0.45,sbY2+0.32,8.2,0.45,{fs:18,bold:true,color:C.cer,cs:3});
...
T([{text:"RECOMMENDATION    ",options:{bold:true,color:C.cer,cs:2}},{text:"Deploy the deterministic migration scheduler; skip joint-covariance and copula modeling; price robustness only past a severity real grids never reach.",options:{bold:true,color:C.white}}],M+0.6,rY,UW-4.95,rH,{fs:25,align:"left",valign:"middle"});
...
pres.writeFile({fileName:OUT+"poster_capstone_v23.pptx"}).then(f=>console.log("WROTE",f));
