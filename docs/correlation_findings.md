...
**Closing pointer (addressed downstream).** The operational results in the thesis are computed on forecast *residuals*, not raw CI: each zone's hour-of-day mean is removed, with seasonal and AR(1) residualizations run as a robustness arm, and the spatial-value null is evaluated on those residuals (see the thesis correlation-validation and spatial-gap sections). So the raw-vs-residual caveat above is resolved in the main analysis; this document is the raw-CI exploratory pass that motivated it, not an open task.
