...
\emph{crossover} under grid-emergency stress, beyond which robustness pays. A
dedicated chapter (Section~\ref{sec:testing}) documents the validation that
underpins every claim: small-instance model checks, a pre-registered falsification
protocol, and a $175$-test software suite.
\end{abstract}
\newpage
\tableofcontents
\newpage
...
small-instance model validation, a pre-registered protocol, and a $175$-test software
...
limitations. Section~\ref{sec:transfer} then develops the active transfer channel
that the null points to, the spatially-coupled transfer DRO and its tail-risk
crossover. Section~\ref{sec:testing} consolidates the model, protocol, and software
validation behind every claim. Section~\ref{sec:conclusion} concludes and outlines
future work, and Appendix~\ref{app:programme} places the thesis within a five-part
research programme.
...
robustness pays under tail risk.} Modelling rare grid-emergency events (with
probability $0.1$ per day a region's carbon is scaled by a severity $M$, as in a
renewable drought or peaker dispatch) and re-solving the two-stage problem with
migration-limited recourse traces a sharp \emph{crossover}: at $M=1$ the robust
commitment is no better than the risk-neutral one (the main null), but as $M$ grows
it increasingly wins, by up to $+8.4\%$ of $\CVaR_{0.95}$ at $M=4$ on the US West.
The robust commitment stays hedged while the risk-neutral one is burned when its
favoured region spikes.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/part3_preliminary.pdf}
\caption{The transfer channel. \textbf{A:} active transfer reduces out-of-sample
...
severity $M$ grows; near $M\approx2$ it turns positive.}
\label{fig:phase3}
\end{figure}
...
These transfer results are a strong proof of concept, and the supporting
optimization is validated code (Section~\ref{sec:testing-transfer}), but the
\emph{experimental protocol} is not yet at the standard of the body's null. The
grid-emergency model is stylised (a Bernoulli severity shock, not a calibrated
outage process), the scenario counts are modest, and the transfer experiments are
not pre-registered. The crossover is therefore a robust qualitative phenomenon whose
precise threshold awaits a calibrated emergency model and a pre-registered
evaluation. We report it because it completes the arc the null opened, and because
its direction, robustness pays only in the tail, is exactly what the mean-dominance
mechanism predicts.
...
The implementation carries a $175$-test suite run under continuous integration on
...
budget, plus a guard rejecting an invalid risk argument. What remains preliminary
about Part~3 (Section~\ref{sec:transfer}) is therefore the experimental protocol, not
the optimisation: the crossover rests on validated code.
...
\section{The Five-Part Research Programme}\label{app:programme}
The body of this extended thesis establishes Parts~1--3 of a planned five-part
programme. \textbf{Part~1} is the covariance null and the mean-dominance bound
(Sections~\ref{sec:results}--\ref{sec:discussion}); \textbf{Part~2} is the copula
extension that forecloses the ``wrong dependence object'' objection
(Section~\ref{sec:res-copula}); \textbf{Part~3} is the spatially-coupled transfer DRO,
the active-decision algorithm and the tail-risk crossover (Section~\ref{sec:transfer}),
included here as a validated proof of concept whose rigorous, pre-registered version,
with a calibrated emergency model and, ideally, affine decision rules for the recourse
policy, is itself the immediate next step.

The programme then continues beyond this thesis. \textbf{Part~4} takes the transfer
model \emph{online}: a multistage / rolling-horizon robust controller in which
forecasts arrive sequentially and demand and transmission are themselves uncertain,
with a Wasserstein ball centred on a \emph{learned} forecast (learning-augmented DRO)
and evaluated closed-loop on real operational traces, the deployable systems
contribution. \textbf{Part~5} generalises the mean-dominance bound itself into a
problem-class condition for \emph{when} cross-coordinate dependence can ever improve a
robust allocation, of which carbon-aware scheduling is one instance and energy
storage, EV fleets and spatial supply chains are others, the theory that turns the
sequence into a research programme. Parts~4--5 are scoped but not yet executed; they
are stated to place the present contribution within its arc, not as claimed results.
The programme is detailed in the project's research roadmap.
