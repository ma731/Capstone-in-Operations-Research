...
by $4.0$--$9.9\%$, and a two-stage robust commitment exposes a tail-risk
\emph{crossover} on the strongly correlated grid, where under severe emergencies the
robust schedule beats the risk-neutral one (a multi-seed-stable effect absent on
structureless grids). Rolling
...
Multi-seed testing (Table~\ref{tab:crossover}) is deliberately conservative and
tightens the claim rather than inflating it: of the three grids, only the strongly
correlated Western US shows a crossover that survives seed variation, and even there
the magnitude is uncertain ($+4.4\pm3.4\%$ at $M=4$). The single-seed positives on the
other grids were scenario-draw noise, the same artifact a day-bootstrap cannot detect,
caught here only by re-seeding the emergency draw. What remains short of the body's
null is therefore both the \emph{emergency model} (a Bernoulli severity shock,
interpretable but not a calibrated outage process) and the magnitude's wide
seed-variance; what is solid is the \emph{direction} and its restriction to grids with
genuine structure. We report the crossover because it completes the arc the null
opened, and because where it appears, robustness paying only in the tail on a
correlated grid, is exactly what the mean-dominance mechanism predicts.
...
\emph{active} transfer decision unlocks $4.0$--$9.9\%$, and under \emph{severe}
grid-emergency stress a tail-risk crossover appears, multi-seed-stable, on the
strongly correlated grid (the robust commitment winning $+4.4\%$ on average at $M=4$)
but absent on structureless grids, exactly where genuine spatial structure exists to
hedge. Part~4 confirms the same logic online: rolled day by day under realistic
forecast error, robustness is null on the correlated grids and counterproductive on
the structureless one, the closed-loop echo of the passive null. Part~5 abstracts the whole pattern into a single
...
\item \textbf{Robustify only against \emph{calibrated tail emergencies}, and only on
grids with genuine structure.} Under normal carbon, robustness is neutral-to-harmful
(Parts~1--4); it earns its cost solely in severe, rare events, and even then
demonstrably only where the grid is correlated enough to hedge (Part~3, multi-seed).
\end{enumerate}
...
(Section~\ref{sec:transfer}) that recovers $4.0$--$9.9\%$ of $\CVaR_{0.95}$ and
exhibits a tail-risk crossover on the correlated grid, the positive resolution the null predicts; and
...
$f_{r\to s,t}$ a decision unlocks $4.0$--$9.9\%$ of $\CVaR_{0.95}$ and, under
grid-emergency stress, reveals the tail-risk crossover on the correlated grid, where robustness pays under severe emergencies.
