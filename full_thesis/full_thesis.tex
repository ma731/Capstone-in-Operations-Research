...
% Front matter (abstract, contents) numbered i, ii, iii ...;
...
then isolate the value of \emph{passive}
...
\textbf{null}: encoding cross-region covariance adds below
...
test. A mean-leveling test shows the covariance signal is genuine but masked by the mean
...
emergency-severity crossover $M^\star\approx3$ (with $M$ the multiple by which a
region's carbon intensity spikes in a rare grid emergency, so $M=3$ is a tripling);
tested against real data-grounded
worst-tail emergencies across 17 zones, that crossover does not activate. This extended
...
pre-committed protocol, and a $202$-test software suite. The thesis contributes a
...
bound, all under 202 tests, with the Phase~1--2 falsification protocol pre-committed (the stylised-emergency crossover of RQ3 is a calibrated stress test, not part of the pre-commitment).
\end{abstract}
\newpage
\tableofcontents
\newpage
...
does not require the regions' power grids to be physically interconnected; cross-region
...
\emph{vector} with a full cross-region covariance, the extension studied here.

\subsection{Problem statement and research questions}
Each modelling layer, cross-region covariance, distributional robustness, and richer
...
The thesis makes five contributions. (i)~\emph{A working day-ahead scheduler with
honest, baseline-anchored savings}, developed in the main body as an active transfer
channel (Section~\ref{sec:transfer}): active inter-region migration cuts out-of-sample
$\CVaR_{0.95}$ by $4.0$--$9.9\%$ over the no-transfer ($\Phi=0$) baseline on the same
feasible set across three real grids, via a spatially-coupled transfer DRO that makes
inter-region flows a decision. The transfer mechanism follows established carbon-aware
practice; the contribution is the $\Phi=0$-anchored measurement of its spatial value.
(ii)~\emph{A pre-committed methodology for pricing modelling layers}: holding the
optimization fixed and manipulating one object at a time, it measures the marginal
out-of-sample $\CVaR$ value of each layer, and separates the \emph{validity} of the
spatial-dependence assumption (correlations are genuine, $0.7$--$0.8$) from its
\emph{value} to the decision (null). To our knowledge this is the first treatment to
place carbon intensity itself in a Mahalanobis--Wasserstein ball as a correlated
cross-region vector and isolate the marginal value of that coupling via a
shuffled-marginals falsification test. (iii)~\emph{A screening rule} for when passive
dependence modelling cannot help, supported from three independent directions: an
a-priori SOCP gap-bound (Proposition~\ref{prop:meandom}) before any data is touched; a
mean-leveling test showing the covariance signal is genuine but masked by the mean field;
and a tail-dependence/comonotone-copula argument showing even the maximal coupling
recovers nothing. A dimensionless condition $\Delta$ (Section~\ref{sec:condition})
generalises the screen beyond carbon, reported as a coarse ordinal indicator (it does
not bind on the real grids, so the empirical falsification remains necessary there).
(iv)~\emph{A price-of-robustness decision rule} with a data-grounded, falsified
crossover, taken online as a rolling-horizon controller (Section~\ref{sec:online}):
robustification pays only past an emergency-severity crossover $M^\star\approx3$ that
realized severity ($1.3$--$1.9\times$; Winter Storm Uri $1.3\times$) does not reach,
stated as the rule plus its tested bound, not a universal impossibility. (v)~\emph{A
reproducible, validated pipeline} (Section~\ref{sec:testing}): pre-commitment, $202$
unit tests, a small-instance model validation, a robustness battery,
Benjamini--Hochberg correction, and per-cell TOST equivalence tests that affirmatively
reject any effect above the $0.4\%$ materiality margin.
...
pre-committed, triangulated \emph{measurement} of the dependence layer's value, not
the qualitative conclusion. This thesis fills that gap: we build a working day-ahead
scheduler
that actively migrates compute and measure the savings (RQ1), isolate the value of
passive modelling sophistication via a falsification test with a mechanistic explanation (RQ2),
...
two further experiments (mean-leveling test; tail dependence) identify the mechanism.
Throughout, a pre-commitment discipline removes analyst degrees of freedom.
...
A California/Nevada subset of the US West (before Phoenix was added at the supervisor's
request) corroborates at the low-correlation end; an \emph{Iberia--France} European set,
explored at the project's outset, is set aside as too weak a test (its residual
correlation collapses to the diurnal cycle and the DRO does not consistently engage
there), so the analysis rests on the US/Canada grids. Temperature for
...
three grids). The theorem therefore bounds the spatial gap by a loose, non-binding
worst-case certificate (right-hand side $7.4$--$12.7\%$ of $\CVaR$, $30$--$55\times$ the
realized gaps) \emph{before any experiment
is run}, and the
...
the sampling-noise floor. The mean-leveling test of Section~\ref{sec:res-ablation}
...
AR(1), two mean-leveling settings, walk-forward, fine grid\}) is on the order of $10^4$ SOCP
...
(it drives the largest mean-leveling test signal, Section~\ref{sec:res-ablation}), whereas
...
cases. Together with the corroborating California/Nevada subset
...
physics: the US West forms one tightly-coupled Western Interconnection cluster, the Ontario--Eastern
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{../figures/correlation_map.png}
...
reading of this number: the saving is a \emph{gross spatial ceiling}. The transfer budget
is set large enough not to bind and migration carries no per-unit, latency, or bandwidth
cost, so $4.0$--$9.9\%$ bounds the value of \emph{perfect, costless} migration rather than
netting out real interconnect economics; introducing a migration cost can only reduce it.
Section~\ref{sec:transfer} develops the full transfer DRO, its multi-seed validation, and
a data-grounded emergency stress test.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{../figures/transfer_value_curve.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{../figures/complexity_frontier.png}
...
Crucially, this is a null of the \emph{strong} kind. In all three pre-committed
...
the cross-region covariance still buys nothing. This is the first regime in the project
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/cv_curve.png}
...
low-correlation null ($\varepsilon^\star=1$ throughout, still no value) that Iberia, where
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/finding.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../figures/schedule_us_west.png}
...
that passes the pre-committed agreement rule (positive and sign-stable across
...
$q=0.05$ across all 144 real-data cells (four grid-runs, the three primary grids
...
pre-committed $144$-cell FDR family of Table~\ref{tab:robust}; for completeness, no
...
of Section~\ref{sec:practical} this is $\approx0.4\%$ of $\CVaR_{0.95}$), and
...
low, heterogeneous-correlation portfolio. Equivalently, the conclusion holds for any
materiality margin $\Delta\ge0.31\%$; the $0.4\%$ figure is fixed a priori from the
economic grounding of Section~\ref{sec:practical}, not reverse-engineered to clear the
test. That last caveat does not threaten the thesis: the
...
\paragraph{The null survives a serial-dependence bootstrap.} The paired bootstrap above
resamples days independently, yet $\CVaR_{0.95}$ averages the year's worst ${\approx}12$
days, and those cluster under multi-day weather events, so an independent-day scheme can
understate the standard error. Re-estimating the spatial-gap intervals with a moving-block
bootstrap that resamples contiguous runs of days ($L=7$ and $L=14$;
the \texttt{block\_bootstrap} snapshot) inflates the standard error by $9$--$44\%$, as
expected, but every grid's gap interval still lies well within the $0.4\%$ materiality
margin (widest bound $0.22\%$). The equivalence conclusion is therefore not an artifact of
the independence assumption: the gaps sit an order of magnitude below materiality, with
room to spare even after the serial-dependence correction.
...
covariance. Under the \textsc{flat} setting (constant scheduling mean, the
...
grid ($-10.34\%$, $[-18.3,-4.4]$, a single representative seed; the four-seed mean is a
milder but still negative $-4.4\%$, Section~\ref{sec:online}): hedging forecast error when
no emergency is present costs tail performance where the regions are weakly coupled. This sharpens the decision
...
zero with unstable sign and does not survive multi-seed testing, so the single-seed
positives there are scenario-seed noise (Section~\ref{sec:transfer} gives the full
four-seed treatment). The crossover is therefore real but grid-specific and only
marginally significant (US West, one-sided), with $M^\star\approx3$ the materially
relevant threshold. The robust schedule stays hedged while the risk-neutral one is
burned when its favoured region spikes.
...
$M^\star\approx3$ (grid-specific); the plotted curves are single representative seeds,
and the multi-seed treatment of Section~\ref{sec:transfer} shows the crossover is
sign-stable only on the US West grid. Realized joint severity stays at $1.3$--$1.9\times$
...
covariance-based ambiguity set improves robust scheduling. The mean-leveling test
...
as ``the data had no signal''; the mean-leveling test shows the signal is present but
...
nothing: the spatial gap is null throughout. Indeed, in our out-of-sample 2025 test
...
walk-forward to 2024, 2023, and 2022 (training only on the years before each) all
reproduce the null, the largest spatial gap across the three earlier test years being
$0.21\%$ of $\CVaR_{0.95}$ (well within the $0.4\%$ margin), though longer multi-year
rolling evaluation would strengthen external validity further. \emph{(iii)~Dependence model.} The DRO uses a
...
mixes (e.g.\ hydro-thermal systems with strong seasonal storage) remain untested. More fundamentally, the result
...
representative run of Table~\ref{tab:online} is more extreme than that spread at
$-10.3\%$), with
...
Section~\ref{sec:testing} does). The Diversified $-10.3\%$ is this seed's value and is
more extreme than the four-seed $S{=}40$ spread (mean $-4.4\%$, range $-1.1$ to
...
(Figure~\ref{fig:part5}), and the pre-committed mean-leveling test confirms the empirical
dependence value rises with it, from $\approx0$ on the real grids to $+1.46\%$ in the
fully mean-leveled world (Section~\ref{sec:res-ablation}).
...
it hurts there) and the genuine signal surfaces only at full leveling ($+0.67\%$ at
...
pre-committed mean-leveling test (a different, tighter feasible set); what transfers is the
...
($\kappa=1$ real grid $\to$ $\kappa\to0$ mean-leveled). $\Delta$ rises monotonically
as the mean weakens, tracking the empirical dependence value ($\approx0$ on real grids,
$+1.46\%$ fully leveled); the shaded band marks the $\Delta<1$ region where dependence is
provably immaterial.}
\label{fig:part5}
\end{figure}
...
\subsection{The falsification protocol and pre-commitment}
The scientific claim is protected by pre-commitment. Each experiment follows a
...
multiple-testing correction (Benjamini--Hochberg over the pre-committed $144$-cell
family) guards the discovery rate.

\subsection{The software test suite}\label{sec:testing-suite}
The implementation carries a 202-test suite (189 run under continuous integration) on
...
flattening operators against hand-computed cases, bringing the suite to $202$ tests.
...
and tail-level sweeps, mean-leveling test. Each file attacks the same null from a new
...
problem fails by a wide margin and the mean-leveled world satisfies (recovering
...
so this is an \emph{active} null: the robustification is on and the cross-region covariance
still adds nothing. A mean-leveling test proves the covariance signal is real ($+1.46\%$ in
...
screening rule is that passive sophistication cannot pay where the diurnal mean field
dominates cross-region dependence, the regime that holds on every grid studied here.
...
materially outperforms the risk-neutral one only past a crossover $M^\star\approx3$
...
\subsection{Contributions}
This thesis contributes: (1)~a pre-committed falsification methodology
...
(3)~a mechanistic explanation (formalized as a mean-dominance bound
(Proposition~\ref{prop:meandom}) and demonstrated by the mean-leveling test) that
explains the null rather than merely reporting it; (4)~a fully reproducible,
version-controlled, CI-tested pipeline (202 tests) with archived license-safe
...
tail alone), with a pre-committed evaluation and affine recourse rules, and test
...
and pre-committing its evaluation, is an open problem we leave to future work.
Further
...
2023, doi:10.1109/TPWRS.2022.3173250.
...
tests in total; CI runs 189, excluding the 13 Electricity Maps integration tests
that require licensed data). Experiments run under Python with CVXPY; solvers are CLARABEL
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../figures/copula_density.png}
...
zero. This measurement is \emph{independent} of the mean-leveling test, so it pins $\Lambda$
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../figures/copula_result.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{../figures/scenario_tail.png}
...
extension, Section~\ref{sec:res-copula}) are at full rigour: pre-committed,
...
pre-committed protocol, ideally with affine recourse rules. \textbf{Part~4} (the
