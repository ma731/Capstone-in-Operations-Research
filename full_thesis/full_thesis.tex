...
protocol, and a $184$-test software suite.
\end{abstract}
\newpage
\tableofcontents
\newpage
...
$\Delta\ll1$. As a \emph{regime indicator} it is monotone: a mean-flattening sweep
($\kappa:1\!\to\!0$) shrinks $M$ and raises $\Delta$ (Figure~\ref{fig:part5}) in
lockstep with the empirical dependence value, which rises from $\approx0$ on the real
grids to $+1.46\%$ in the mean-ablated world (Section~\ref{sec:res-ablation}). The
condition correctly \emph{orders} the regimes; calibrating an absolute threshold is
future work. The diagnostic transfers beyond carbon: cross-coordinate dependence is
worth modelling only where the mean field is weak relative to the residual structure,
