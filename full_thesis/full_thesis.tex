...
dependence modelling cannot help, certified from three independent directions: an
a-priori SOCP gap-bound (Proposition~\ref{prop:meandom}) before any data is touched; a
mean-ablation showing the covariance signal is genuine but masked by the mean field;
...
spatial correlation.
\item[Mean-ablation (the mean-leveling test):] we deliberately level each region's average carbon so the only signal left is the cross-region structure; it reveals whether the dependence is real but masked by the mean field. (``Ablation'' is the machine-learning term for removing one component to measure its effect.)
...
\paragraph{Mean-ablation (RQ3, mechanism).} To test whether the mean field masks the
...
$0.23\%$, two orders of magnitude smaller. The theorem therefore bounds the spatial
gap by a loose, non-binding worst-case certificate (right-hand side $7.4$--$12.7\%$ of
$\CVaR$, two orders of magnitude above the realized gaps) \emph{before any experiment
is run}, and the
