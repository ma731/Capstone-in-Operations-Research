...
by $4.0$--$9.9\%$. Whether \emph{robustifying} that transfer pays is then the sharper
question, and the answer is a deeper null: a tail-risk \emph{crossover} appears only
under \emph{synthetic} stress more severe than the historical record (and only
marginally, on the correlated grid), and vanishes once emergencies are drawn from the
data's real upper tail. Rolling the controller \emph{online} under forecast error
likewise leaves robustness immaterial (Section~\ref{sec:online}), and a dimensionless
...
pre-registered protocol, and a $194$-test software suite.
\end{abstract}
\newpage
\tableofcontents
\newpage
...
engineered low, heterogeneous-correlation solar/wind/hydro portfolio) corroborated by an
...
$4.0$--$9.9\%$ of $\CVaR_{0.95}$ and exhibits a tail-risk crossover on the correlated grid under
...
small-instance model validation, a pre-registered protocol, and a $194$-test software
...
spatial value.} Turning the transfer budget from off to free cuts out-of-sample
$\CVaR_{0.95}$ by $4.0\%$ (Western US), $9.0\%$ (Diversified) and $9.9\%$ (Eastern
US--Canada) (\texttt{run\_part3\_transfer\_value.py}), the value the passive null left
on the table, captured by exploiting the spatial \emph{mean} (shipping work to the
region cleanest on average). \textbf{(2)~Robustness is worthless under normal carbon, even here.} A
...
spatial structure an \emph{active} transfer decision unlocks $4.0$--$9.9\%$
\emph{deterministically} (by exploiting the spatial mean), while \emph{robustifying}
...
DRO is empirically sound; the engineered heterogeneous portfolio (pairwise
correlations spanning $0.00$ to $0.37$, mean $\approx0.2$) confirms the diagnostic
resolves a genuinely near-zero \emph{pair} (ERCOT--BPAT, $0.00$) alongside moderate
ones, rather than washing everything to a single value.
...
(Section~\ref{sec:transfer}) that recovers $4.0$--$9.9\%$ of $\CVaR_{0.95}$
\emph{deterministically}, the positive resolution the null predicts, while showing
...
$f_{r\to s,t}$ a decision unlocks $4.0$--$9.9\%$ of $\CVaR_{0.95}$ deterministically;
robustifying it, by contrast, pays only under synthetic stress beyond the historical
record (the crossover does not survive data-grounded emergencies). The immediate future
work is therefore to settle whether the crossover is ever real: build a \emph{calibrated}
outage model from grid-operator data (not the historical carbon tail alone), with a
pre-registered evaluation and affine recourse rules, and test whether genuine emergency
risk reaches the severity the crossover needs. To our knowledge no prior work combines Wasserstein-DRO
