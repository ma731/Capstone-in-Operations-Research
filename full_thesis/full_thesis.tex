...
pre-registered protocol, and a $199$-test software suite. The thesis contributes a
...
bound, all under 199 unit tests, with the Phase~1--2 falsification protocol pre-registered (the stylised-emergency crossover of RQ3 is a calibrated stress test, not part of the pre-registration).
\end{abstract}
\newpage
\tableofcontents
\newpage
...
reproducible, validated pipeline} (Section~\ref{sec:testing}): pre-registration, $199$
...
suite, $199$ unit tests, including a feasible-set equivalence test pinning the
...
The implementation carries a 199-test suite (186 run under continuous integration) on
...
flattening operators against hand-computed cases, bringing the suite to $199$ tests.
...
version-controlled, CI-tested pipeline (199 tests) with archived license-safe
...
version-controlled repository with a continuous-integration suite (199 unit
