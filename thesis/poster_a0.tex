...
\settitle{
  ...
  \begin{minipage}[c]{0.17\linewidth}\centering
    \IfFileExists{../figures/ie_logo.pdf}{%
      \colorbox{white}{\rule[-1.7cm]{0pt}{0pt}\includegraphics[height=4.0cm]{../figures/ie_logo.pdf}}}{%
      \IfFileExists{../figures/ie_logo.png}{%
        \colorbox{white}{\rule[-1.7cm]{0pt}{0pt}\includegraphics[height=4.0cm]{../figures/ie_logo.png}}}{%
...
\block{\centering Background \& research questions}{
...
\begin{itemize}
\item Pre-committed: config commit-locked, single 2025 test read.
...
walk-forward to 2024, tighter-ramp and utilization sweeps, and the full tail range
$\mathrm{CVaR}_{0.90}$--$\mathrm{CVaR}_{0.99}$. A per-cell \textbf{equivalence test
(TOST)} affirmatively rejects any effect above the $0.4\%$ materiality margin in all
$27$ cells, the gap is statistically $\mathbf{=0}$, not merely undetected.
\vspace{0.2cm}
\centering
...
\block{Conclusions \& the active extension}{
\begin{itemize}
...
inter-region transfer channel $f_{r\to s,t}$. It cuts $\mathbf{4}$--$\mathbf{10\%}$
\emph{deterministically} (by exploiting the spatial mean), but \emph{robustifying}
it pays nowhere data-grounded, online or under real tail-day emergencies. The null
extends to the active and online settings: value lives in the decision, not the
dependence model.
\end{itemize}
\vspace{0.2cm}
\small Reproducible pipeline: \textbf{202 unit tests}, pre-committed configs,
per-cell equivalence tests, multi-seed stability, an independent adversarial review,
license-safe archived results. \emph{AI used as a coding/drafting assistant under the
author's direction.}
}
