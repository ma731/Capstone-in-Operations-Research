...
ball keeps the program an SOCP, but now the cross-region covariance enters the
\emph{decision}, not merely a passive penalty. This is the genuinely novel
formulation the present results motivate: the mean-dominance bound
(Proposition~\ref{prop:meandom}) shows passive dependence modelling cannot help, so
the only route to spatial value is to make the spatial structure actionable. To our
...
scope and awaits supervisor sign-off. Third, \emph{rolling re-optimization} over multiple
