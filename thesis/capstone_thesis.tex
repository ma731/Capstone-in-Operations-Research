...
$q=0.05$ across all 144 non-ablation cells (the $3\text{ cases}\times3\text{
regimes}\times3\,\alpha=27$ base cells $\times$ the four covariance estimators
\{sample, Ledoit--Wolf, seasonal, AR(1)\}, plus the two corroborating panels' cells)
leaves that same already-rejected cell as the largest surviving positive gap, nothing material survives both filters.
...
every spatial gap sits inside the materiality band.
...
cells and the tick marks the median. The shaded band is the $\pm0.1\%$ materiality
...
spatial advantage \emph{an order of magnitude below} the $\approx1\%$ materiality
threshold, and roughly the same order as the per-cell gaps themselves, yet no
sign-stable positive gap of even this size survives. The null is a genuine absence
...
Section~\ref{sec:method-copula}. This is the most any copula can buy. At the locked
...
robust scheduler, one ambiguity set per zone, no cross-region covariance, captures essentially all the achievable robust value at a fraction of the
...
\$$80/\text{tCO}_2$ price, which is $0.04\%$ of the facility's $\approx$\$$84$M
...
intact, but a qualitatively different objective (e.g.\ hard per-job deadlines) is
untested.
...
with $S$; the $\pm0.1\%$ materiality band is shaded.}
\label{fig:converge}
\end{figure}
...
stylised grid-emergency stress model, modest scenario counts, no pre-registration ---
and a rigorous treatment is the subject of a follow-on study; we include them only because
...
bound). \textbf{Part~3}, sketched above, is the spatially-coupled transfer DRO ---
the active-decision algorithm and the tail-risk crossover --- promoted to a rigorous,
