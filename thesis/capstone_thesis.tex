...
\paragraph{Why the dependence model is second-order: a bound.} A structural reason
to expect Phase 2 to reproduce the null follows from the translation invariance of
CVaR. Writing the carbon field as mean plus zero-mean residual,
$\rho=\bar\rho+\xi$, daily emissions decompose as $\langle\rho,x\rangle=
\langle\bar\rho,x\rangle+\langle\xi,x\rangle$, and since $\langle\bar\rho,x\rangle$
is deterministic,
\begin{equation}
\CVaR_\beta\!\big(\langle\rho,x\rangle\big)=
\underbrace{\langle\bar\rho,x\rangle}_{\text{copula-independent}}+\;
\CVaR_\beta\!\big(\langle\xi,x\rangle\big).
\label{eq:meandom}
\end{equation}
The dependence model affects \emph{only} the residual term, and only through its
cross-region aggregation; its total leverage is bounded by the comonotone--diversified
spread $\Lambda(x)=\sup_{C}\CVaR_\beta(\langle\xi,x\rangle)-\inf_{C}
\CVaR_\beta(\langle\xi,x\rangle)$ over copulas $C$.

\begin{proposition}[Mean-dominance]\label{prop:meandom}
For any two dependence models, the optimal out-of-sample $\CVaR_{0.95}$ differs by
at most $\Lambda(x^\star)$. The mean-ablation of Section~\ref{sec:res-ablation}
measures $\Lambda$ directly: it is the gap recovered when the mean term is removed.
Empirically $\Lambda$ is at most a few tenths of one percent of the mean term with
the mean present, rising to $\approx1.5\%$ only under flat ablation; hence the mean
field pins the schedule and the dependence model --- elliptical or not --- is
immaterial.
\end{proposition}

\noindent The proposition predicts that even Clayton, the correct object for the
empirical asymmetry, cannot recover material value; Section~\ref{sec:res-copula}
confirms it.
...
(Table~\ref{tab:copula}, Figure~\ref{fig:copularesult}), no model beats the
independence baseline by a material, sign-stable margin: across both arms the
largest absolute copula gap anywhere is $0.27\%$ of $\CVaR_{0.95}$ (the Gaussian
arm on \texttt{us\_hetero}; the lower-tail Clayton arm peaks at $0.13\%$), with
signs flipping across $\alpha$ exactly as in Phase 1. The null is now \emph{total}
--- it holds not just for the covariance ball but for the full copula.
...
$\CVaR_{0.95}$ versus the independence baseline (\%; positive $=$ the structured
copula helps); ``Clay$-$Gauss'' is the mean asymmetry increment. No case shows a
material, sign-stable copula gain. $\theta$ is the fitted Clayton parameter.}
\label{tab:copula}
\begin{tabular}{lcccccc}
\toprule
Case & $\bar\tau$ & $\theta$ & max$|$gap$_{\text{Gauss}}|$ &
max$|$gap$_{\text{Clay}}|$ & mean (Clay$-$Gauss) & detectable\\
\midrule
\texttt{us\_west}   & 0.47 & 1.77 & 0.066 & 0.038 & $+0.011$ & none stable\\
\texttt{taskc}      & 0.31 & 0.90 & 0.158 & 0.083 & $-0.056$ & none stable\\
\texttt{us\_hetero} & 0.20 & 0.50 & 0.270 & 0.127 & $+0.070$ & none stable\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Phase 2 result. \textbf{A:} maximum $|$gap$|$ versus independence for the
Gaussian and Clayton arms --- neither copula materially beats the decoupled
baseline. \textbf{B:} the mean Clayton$-$Gaussian increment by grid; it is positive
where dependence is heterogeneous (\texttt{us\_hetero}, $+0.07\%$), confirming the
copula \emph{does} capture the asymmetry --- but the effect is an order of magnitude
