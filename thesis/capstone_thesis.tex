...
\IfFileExists{../figures/ie_logo.pdf}{%
  \includegraphics[height=2.2cm]{../figures/ie_logo.pdf}\par\vspace{0.8cm}}{%
  \IfFileExists{../figures/ie_logo.png}{%
    \includegraphics[height=2.2cm]{../figures/ie_logo.png}\par\vspace{0.8cm}}{}}
...
\caption{The spatial null, seen directly (\texttt{us\_west}, regime R3,
...
curtailed. Figure~\ref{fig:robust} collects the whole battery: across all six arms
and three grids, every spatial gap sits inside the materiality band.
...
threshold. No arm produces a sign-stable, material gap; the few \texttt{us\_hetero}
...
scenario seed its largest single-cell gain is $+0.18\%$ (\texttt{taskc}), and a
naive day-bootstrap would call several \texttt{taskc} cells ``detectable.'' That
...
\midrule
\texttt{us\_west}   & 0.47 & 1.77 & 0.066 & 0.038 & 0.089 & $\approx0$, noise\\
\texttt{taskc}      & 0.31 & 0.90 & 0.158 & 0.014 & 0.182 & $[-0.08,+0.10]$, mean $+0.01$\\
\texttt{us\_hetero} & 0.20 & 0.50 & 0.069 & 0.127 & 0.114 & $\approx0$, noise\\
\bottomrule
\end{tabular}
\end{table}
...
$\sup_C$) arms. The largest single-seed comonotone gain is $0.18\%$ (\texttt{taskc});
...
grid; positive where dependence is heterogeneous (\texttt{us\_hetero}, $+0.07\%$),
...
(\texttt{taskc}). Distribution of real 2025 daily emissions under the schedule
...
\texttt{taskc} over five scenario seeds and a range of scenario counts $S$. The
...
\texttt{taskc}, against the scenario count $S$. The band straddles zero and narrows
