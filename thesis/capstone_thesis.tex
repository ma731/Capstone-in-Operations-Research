...
human-checkable instance (one region, eight hours, $10$ MWh per-cell capacity,
$W=35$ MWh), rather than each constraint in isolation: correctness is a property of
the constraints' \emph{interaction}, and a set that every constraint satisfies
alone can still be jointly binding in unexpected ways. We fix an analyst-anticipated
outcome, perturb one input at a time, and verify the solved schedule moves the
anticipated way (Figure~\ref{fig:modelval}). Under a clean carbon field the load
concentrates in the cheapest hours subject to the ramp and deadline (panel A).
Making two of those hours dirty drives the load to the next-cleanest feasible hours,
work still conserved and the deadline still met (panel B). Tightening the ramp limit
from $6$ to $3$ MW/h removes the model's ability to jump to capacity, so the schedule
smooths into a gradual plateau (panel C). All three responses match the
hand-anticipated solution and conserve work to machine precision. This model-level
...
\caption{Full-model validation on a small instance. The complete feasible set
(capacity, work conservation, ramp, deferral deadline) is solved jointly; bars are
the optimal schedule $x^\star$, the gold curve the carbon field, the green band the
deadline window. \textbf{A:} baseline. \textbf{B:} after making hours 3--4 dirty,
load migrates to the next-cleanest hours. \textbf{C:} after tightening the ramp
limit, the schedule can no longer spike and ramps gradually. Each response is the
one an analyst anticipates, confirming the model encodes the intended behaviour.}
\label{fig:modelval}
\end{figure}
...
against the economic materiality margin $\Delta=0.4\%$ of $\CVaR_{0.95}$ (the
energy-bill threshold of Section~\ref{sec:practical}; the $\approx1\%$ practitioner
threshold is looser still). For each grid we take its most adversarial observed gap
and the paired-bootstrap standard error of Table~\ref{tab:power}, and test
$H_0\!:\lvert\text{gap}\rvert\ge\Delta$ against $H_1\!:\lvert\text{gap}\rvert<\Delta$.
The $90\%$ confidence interval lies strictly inside $(-\Delta,+\Delta)$ in every
grid. Even the worst case (Diversified, gap $=-0.233\%$, $\mathrm{SE}=0.039\%$, CI
$[-0.297,-0.169]\%$) clears both one-sided tests at the $5\%$ level, the binding
statistic being $z=(\Delta-\lvert\text{gap}\rvert)/\mathrm{SE}=4.3$ ($p<10^{-4}$);
against the looser $1\%$ margin every grid gives $z\ge19$. We therefore do not merely
fail to detect a spatial benefit; we statistically reject the existence of one (in
either direction) larger than the materiality threshold. That is the strongest form a
null can take.
...
again be second-order. Preliminary, proof-of-concept experiments
(Appendix~\ref{app:phase3}) already bear this out and sharpen it: active transfer
cuts out-of-sample $\CVaR_{0.95}$ by $4.7$--$10.1\%$; robustness stays immaterial
