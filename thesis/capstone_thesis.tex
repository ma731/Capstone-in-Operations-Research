...
validated pipeline}: pre-registration, $194$ unit tests, a small-instance model
...
three grids). The bound holds \emph{before any experiment
...
and robust transfer pays only in the tail (Section~\ref{sec:res-crossover}). Three forces
make the transfer saving mean-driven and keep passive covariance from adding to
it: the within-day swing of mean intensity dwarfs the residual covariance; the
strongest correlation (US West) is common-mode and offers no hedge; and the
...
of Section~\ref{sec:practical}, $\approx0.4\%$ of the energy bill), and
...
rule; and (5)~a reproducible, version-controlled, CI-tested pipeline (194 tests)
