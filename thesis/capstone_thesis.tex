...
migration cuts daily $\CVaR_{0.95}$ emissions by $4.7$--$10.1\%$, an $8$ to $11$
percentage-point spatial lever, and this deterministic transfer channel is the
...
$4.7$--$10.1\%$ ($8$ to $11$ percentage points) over the carbon-aware no-transfer
...
spans the continent with mixed edges, one near-zero and two moderate, by construction.
...
channel} (Appendix~\ref{app:phase3}), at a fraction of the modelling and estimation
...
diagnostic resolves a genuinely near-zero \emph{pair} (ERCOT--BPAT, $0.00$) alongside moderate ones (mean $\approx0.2$), not a single washed-out value.
...
Second, and most promising, a \emph{spatially-coupled transfer DRO}: introduce
inter-region load flows $f_{r\to s,t}\ge0$ as decision variables, so that the work
served in a zone, $\tilde x_{r,t}=x_{r,t}+\sum_{s}(f_{s\to r,t}-f_{r\to s,t})$, can
migrate toward whichever region is cleanest at each hour, subject to flow
conservation, transfer-capacity and (optionally) transmission-loss constraints.
Minimizing the worst-case $\CVaR$ of $\langle\rho,\tilde x\rangle$ over a Wasserstein
ball keeps the program an SOCP, but now the spatial structure enters the
\emph{decision}, not merely a passive penalty. The motivation is precise: the mean
field dominates (Proposition~\ref{prop:meandom}), and it dominates
\emph{spatially} too, since at each hour one region is cleanest on average. A
passive ambiguity set cannot act on that first-order spatial signal; an active
transfer channel can, which is exactly the value our null leaves on the table. The
honest open question, and the real content of the next phase, is not whether
transfer helps (it must, being mean-exploiting) but whether \emph{robustifying} it
beats plain deterministic transfer, e.g.\ under forecast error or for tail-averse
operators, since the same mean-dominance logic suggests the robustness layer may
again be second-order. Preliminary experiments on a unit-tested transfer
...
$4.7$--$10.1\%$ across the three grids, the value the passive null left on the
