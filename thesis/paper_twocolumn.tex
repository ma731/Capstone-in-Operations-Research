% Two-column academic-paper version of the IE Research Capstone thesis.
% Ported faithfully from capstone_thesis.tex (single-column report).
...
\usepackage[letterpaper,margin=0.8in,columnsep=0.26in]{geometry}
...
\title{\vspace{-2.2em}\bfseries The Price of Sophistication: A Pre-Committed Falsification Test for When Spatial and Robust Modelling Pay in Carbon-Aware Data-Center Scheduling\thanks{\textbf{AI Use Declaration.} Generative AI tools were used as a research and engineering assistant under the author's direction: for code implementation support, experiment orchestration, literature triage, and drafting assistance. All research questions, modelling decisions, experimental designs, and interpretations are the author's, developed with the supervisor; all results were produced by version-controlled, pre-committed code reviewed by the author, and all text was reviewed and edited by the author.}}
\author{%
  Marco Ortiz Togashi\\
  \normalsize\texttt{marco.ortiztogashi@student.ie.edu}\\
  \normalsize IE University, School of Science and Technology\\
  \normalsize Madrid, Spain
  \and
  Prof.\ Bissan Ghaddar~(Supervisor)\\
  \normalsize\texttt{bghaddar@faculty.ie.edu}\\
  \normalsize IE University, School of Science and Technology\\
  \normalsize Madrid, Spain}
\date{June 2026}
\twocolumn[
  \begin{@twocolumnfalse}
    \maketitle
    \begin{abstract}\noindent
    Data-center operators can shift compute toward cleaner hours and regions,
...
tested, data-grounded bound, all under 202 tests, with the Phase~1--2 falsification protocol pre-committed (the stylised-emergency crossover of RQ3 is a calibrated stress test, not part of the pre-commitment).
    \end{abstract}
    \vspace{1.4em}
  \end{@twocolumnfalse}
]
...
spatial value. (ii)~\emph{A pre-committed methodology for pricing modelling layers}:
...
validated pipeline}: pre-commitment, $202$ unit tests, a small-instance model
...
$0.23\%$, two orders of magnitude smaller. The theorem bounds the spatial gap by a
loose, non-binding worst-case certificate (right-hand side $7.4$--$12.7\%$ of $\CVaR$,
two orders of magnitude above the realized gaps) \emph{before any experiment is run};
what it buys is not tightness
...
suite, $202$ unit tests total ($189$ run in CI; the 13 Electricity Maps integration tests need licensed data and are excluded from CI), including a feasible-set equivalence test pinning the
...
The geography matches the physics: the US West forms one tightly-coupled Western Interconnection
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{../figures/transfer_value_curve.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{../figures/complexity_frontier.png}
...
the pre-committed $144$-cell FDR family of Table~\ref{tab:robust}; no sweep cell is
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\columnwidth]{../figures/tail_dependence_taskc.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{../figures/crossover.png}
...
mechanistic explanation (formalized as the mean-dominance bound Proposition~\ref{prop:meandom}
and demonstrated by mean-ablation) that explains the null and supplies the screening
rule; and (5)~a fully reproducible, version-controlled, CI-tested pipeline (202 tests total, 189 run in CI)
...
error, and pre-committing its evaluation, is an open problem we leave to future
work. Further
...
\medskip\noindent\emph{Note on AI use.} Generative AI tools were used as a coding and
drafting assistant under the author's direction (see the title footnote); all research
questions, modelling decisions, experimental designs, interpretations, and final text
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\columnwidth]{../figures/scenario_convergence.png}
...
version-controlled repository with a continuous-integration suite ($189$ unit
tests run in CI of $202$ total; the 13 Electricity Maps integration tests are excluded from CI as they require
...
pre-committed study with calibrated emergencies and, ideally, affine decision rules
...
direction, as declared on the title; all research questions, modelling
